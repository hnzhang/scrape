...
if(!empty($_POST)) {
	...
	if(filter_var($val, FILTER_VALIDATE_EMAIL)) {
		if(isSuperEmail($val)){
				header("Location: http://giftcards.surreyknights.net/test/giftcards_report.php?SuperEmail=".$val);
				die();
		}
		else {
			...
			//if(count($orders) === 0){// no order before
				$AccountEmail = $val;
				header("Location: http://giftcards.surreyknights.net/test/giftcards_ordering.php?AccountEmail=".$AccountEmail);
				die();
